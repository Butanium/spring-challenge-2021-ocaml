...
(* TODO (1) optimize plants at the end, we need less seeds and more trees
...
let getSelfShadowSize tree treeSize =
    let rec aux (pos:int) dir depth acc =
        ...
        if n = -1 || depth > treeSize then acc else (
            aux n dir (depth + 1) @@
            match getTreeOpt n with
            (* if t.size < depth sen less malus *)
            | Some t when t.ismine && t.size <= treeSize -> acc + t.size
            | Some t when not t.ismine && t.size < treeSize -> acc - t.size
            | _ -> acc
        )
    and loop i acc = if i = 6 then acc else
        loop (i+1) (acc + aux tree i 1 0)
